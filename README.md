...
You need a current Claude Code or Codex installation. For the full four-model review, install and sign in to the Claude Code, Codex, and Cursor command-line tools. The Grok lane runs on Cursor's hosted Grok, so `cursor-agent` is the tool to install for it. [Bun](https://bun.sh) runs the small local tool that starts models outside the app you are using. You can still use the core workflows with fewer models.
...
| Other models | Codex and Cursor run through their signed-in command-line tools. | Claude and Cursor run through their signed-in command-line tools. |
| Skills and workflows | Shared with Codex. | Shared with Claude Code. |

Grok can take part in a multi-model review through the Cursor command-line tool. You cannot use Grok or Cursor as the main app running pstack.
