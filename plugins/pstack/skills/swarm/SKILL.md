...
4. Pick the worker descriptor from `swarm workers` in the current harness's pstack model sheet when present. Otherwise use `cursor:cursor-grok-4.6-xhigh@xhigh`. For a model race, name each arm's descriptor up front.
